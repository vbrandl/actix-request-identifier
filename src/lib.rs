...
type Generator = Box<dyn Fn() -> HeaderValue>;
...
impl RequestIdentifier {
    ...
    pub fn with_header(header_name: &'static str) -> Self {
        Self {
            header_name,
            id_generator: Box::new(default_generator),
        }
    }
    ...
    /// time it is invoked.
    pub fn with_generator<G>(id_generator: G) -> Self
    where
        G: 'static + Fn() -> HeaderValue,
    {
        Self {
    ...
    pub fn generator<G>(self, id_generator: Generator) -> Self
    where
        G: 'static + Fn() -> HeaderValue,
    {
        Self {
